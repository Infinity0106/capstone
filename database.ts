...
export async function createTable() {
  return new Promise((resolve, reject) => {
    db.transaction(
      (tx) => {
        tx.executeSql(
          "create table if not exists menuitems (id integer primary key not null, title text, price text, category text, description text, image text);"
        );
      },
      reject,
      () => {
...
export async function getCategories() {
  return new Promise<MenuItem["category"][]>((resolve) => {
    db.transaction((tx) => {
      tx.executeSql(
        ...
        (_, { rows }) => {
          resolve(
            rows._array.map((ele) => ele.category) as MenuItem["category"][]
          );
        }
      );
    });
  ...
  return new Promise((resolve, reject) => {
    db.transaction(
      (tx) => {
        ...
        tx.executeSql(
          `insert into menuitems (title, price, category, description, image) values ${menuItems
            .map(
              (item) =>
...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise<MenuItem[]>((resolve, reject) => {
    db.transaction((tx) => {
      if (!activeCategories.length && !query) {
        tx.executeSql("select * from menuitems", [], (_, { rows }) => {
          resolve(rows._array as MenuItem[]);
        });
        return;
      ...
      if (!activeCategories.length && query) {
        tx.executeSql(
          `select * from menuitems where (title like '%${query}%')`,
          [],
          (_, { rows }) => {
            resolve(rows._array as MenuItem[]);
          }
      ...
      tx.executeSql(
        `select * from menuitems where (${activeCategories
          ...
          .join(" or ")}) ${!query ? "" : ` and (title like '%${query}%')`}`,
        [],
        (_, { rows }) => {
          resolve(rows._array as MenuItem[]);
        }
